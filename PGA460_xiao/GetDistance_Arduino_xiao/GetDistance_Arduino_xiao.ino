...
void initPGA460() {
    ...
    SerialUSB.begin(baudRate); // initialize COM UART serial channel
    delay(1000);
    SerialUSB.println("PGA460-Q1 EVM UART/TCI/OWU/SPI Energia Demo for Ultrasonic Time-of-Flight");
    SerialUSB.println("-------------------------------------------------------------------------");
    SerialUSB.println("Instructions: Configure the EVM by entering a byte value between 0-9 or 'x' per request.");
    SerialUSB.println("--- Input can be entered as a single string to auto-increment/fill each request. E.g. 0011211000510");
    SerialUSB.println("--- To skip the COM setup at any point, and use the hard-coded values from thereon, enter a value of 's'.");
    SerialUSB.println("--- To reset the program, and re-prompt for user input, enter a value of 'q'.");
    SerialUSB.println("--- To pause/resume the program when running, enter a value of 'p'.");
    
    int numInputs = 13;
    for (int i=0; i<numInputs; i++) //int i
    {
      switch(i)
      {
        case 0: SerialUSB.print("1. Communication Mode: 0=UART, 1=TCI, 2=OneWireUART, 3=SPI ... "); break;     
        case 1: 
          if(commMode == 0 || commMode == 1 || commMode == 2) // UART, OWU, or TCI
          {
            SerialUSB.print("2. UART kBaud: 0=9.6, 1=19.2, 2=38.4, 3=57.6, 4=74.8, 5=115.2 ... "); break;
          }
          else if (commMode == 3) // SPI
          {
            SerialUSB.print("2. Clock Divider (MHz): 0=16/1, 1=16/2, 2=16/4, 3=16/8, 4=16/16, 5=16/32, 6=16/64, 7=16/128 ... "); break;
          }                          
        case 2: SerialUSB.print("3. P1 and P2 Thresholds: 0=%25, 1=50%, or 2=75% of max ... "); break;   
        case 3: SerialUSB.print("4. Transducer Settings: 0=Murata MA58MF14-7N, 1=Murata MA40H1S-R, x=Skip ... "); break;  
        case 4: SerialUSB.print("5. TVG Range: 0=32-64dB, 1=46-78dB, 2=52-84dB, or 3=58-90dB, x=Skip ... "); break;   
        case 5: SerialUSB.print("6. Fixed TVG Level: 0=%25, 1=50%, or 2=75% of max, x=Skip ... "); break;     
        case 6: SerialUSB.print("7. Minimum Distance = 0.1m * BYTE ... "); break; 
        case 7: SerialUSB.print("8. Run System Diagnostics?: 0=No, 1=Yes ... "); break;
        case 8: SerialUSB.print("9. Echo Data Dump: 0=None, 1=P1BL, 2=P2BL, 3=P1LO, 4=P2LO,... "); break;
        case 9: SerialUSB.print("10. Burn/Save User EEPROM?: 0=No, 1=Yes ... "); break;
        case 10: SerialUSB.print("11. Command Cycle Delay: 100ms * BYTE ... "); break;
        case 11: SerialUSB.print("12. Number of Objects to Detect (1-8) = BYTE ... "); break;
        case 12: SerialUSB.print("13. USART Address of PGA460 (0-7) = BYTE ... "); break;
      }
    ...
    SerialUSB.println("Configuring the PGA460 with the selected settings. Wait...");
    delay(300);
...
*-------------------------------------------------------------------*/
  ...
  // -+-+-+-+-+-+-+-+-+-+- 5 : run system diagnostics   -+-+-+-+-+-+-+-+-+-+- //
    ...
    {      
      ...
      SerialUSB.print("System Diagnostics - Frequency (kHz): "); SerialUSB.println(diagnostics);
      diagnostics = ussc.runDiagnostics(0,1);       // do not re-run system diagnostic, but print decay diag result
      SerialUSB.print("System Diagnostics - Decay Period (us): "); SerialUSB.println(diagnostics);
      diagnostics = ussc.runDiagnostics(0,2);       // do not re-run system diagnostic, but print temperature measurement
      SerialUSB.print("System Diagnostics - Die Temperature (C): "); SerialUSB.println(diagnostics);
      diagnostics = ussc.runDiagnostics(0,3);       // do not re-run system diagnostic, but print noise level measurement
      SerialUSB.print("System Diagnostics - Noise Level: "); SerialUSB.println(diagnostics);
    }
  // -+-+-+-+-+-+-+-+-+-+- 6 : burn EEPROM   -+-+-+-+-+-+-+-+-+-+- //
    if(burn == 1)
    {
      byte burnStat = ussc.burnEEPROM();
      if(burnStat == true){SerialUSB.println("EEPROM programmed successfully.");}
      else{SerialUSB.println("EEPROM program failed.");}
    }
  // -+-+-+-+-+-+-+-+-+-+- 7 : capture echo data dump   -+-+-+-+-+-+-+-+-+-+- //
    if (edd != 0)                                   // run or skip echo data dump
    {
      SerialUSB.println("Retrieving echo data dump profile. Wait...");
      ussc.runEchoDataDump(edd-1);                  // run preset 1 or 2 burst and/or listen command
      SerialUSB.print(ussc.pullEchoDataDumpBulk());
      SerialUSB.println("");
      digitalWrite(0,LOW);
    ...
    // -+-+-+-+-+-+-+-+-+-+-  PRESET 1 (SHORT RANGE) MEASUREMENT   -+-+-+-+-+-+-+-+-+-+- //
      ...
      {      
        ...
        if (distance > minDistLim && distance < 11.2)  // turn on DS1_LED if object is above minDistLim
        {
            ...
            SerialUSB.print("P1 Obj"); SerialUSB.print(i+1); SerialUSB.print(" Distance (m): "); SerialUSB.println(distance);
    ...
    // -+-+-+-+-+-+-+-+-+-+-  PRESET 2 (LONG RANGE) MEASUREMENT   -+-+-+-+-+-+-+-+-+-+- //
      ...
      {   
        ...
        {  
          ...
          {
              //ussc.toggleLEDs(HIGH,LOW,LOW);
              SerialUSB.print("P2 Obj"); SerialUSB.print(i+1); SerialUSB.print(" Distance (m): "); SerialUSB.println(distance);
...
void serialEvent() {
    while (SerialUSB.available()) {
      char inChar = (char)SerialUSB.read(); // get the new byte
      ...
      else if (inChar == 'p')
      {
        ...
        while(stringComplete == false)
        {
          while (SerialUSB.available())
          {
            inChar = (char)SerialUSB.read(); // get the new byte
            if (inChar == 'p')
              {
                stringComplete = true;
              }
