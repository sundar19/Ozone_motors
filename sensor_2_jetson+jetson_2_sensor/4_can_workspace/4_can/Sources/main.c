...
/********* Enable External Interrupt *********/
   EnableExternalInterrupts();
	...
	GPIO_SetState(70, 1);
	GPIO_SetState(71, 1);
	SetCanRxFilter0(1, 0, 0); //CAN_0 receive FILTER
	SetCanRxFilter(1, 0, 0); //CAN_1 receive FILTER
	SetCanRxFilter2(1, 0, 0); //CAN_2 receive FILTER
	SetCanRxFilter3(1, 0, 0); //CAN_3 receive FILTER
	UartBufInit();
	PrintMenu();
   while(1)
   {
...
void ProcessCAN(void)
{
	can_msg_struct msgCanRX,msgCanRX0,msgCanRX2,msgCanRX3;
    
	
	if (CanRxMbFull0(0) == 1)	// Check if CAN message received 
	{
	 ...
	 if(CanRxMbFull2(0) == 1)
	{
	 ...
	 if(CanRxMbFull3(0) == 1)
	{
		msgCanRX3 = CanRxMsg3(0);
		for (k=0;k<=7;k++)
		{
			CANcheck3[k] = msgCanRX3.data[k];
		}
		CanTxMsg0 (2, 1, 8,(uint8_t *)CANcheck3, 0);
	}
