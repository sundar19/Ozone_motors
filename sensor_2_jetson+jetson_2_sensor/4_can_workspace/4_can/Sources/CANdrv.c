...
*
******************************************************************************/
void SetCanRxFilter2(uint32_t id, uint8_t mb, uint8_t ext) 
{
    if (ext)
    {
        CAN_2.BUF[mb].CS.B.IDE = 1;/* MB for extended ID */
        CAN_2.BUF[mb].ID.R = id;   /* MB ID */
    }
    else
    {
        CAN_2.BUF[mb].CS.B.IDE = 0; /* MB for standard ID */
        CAN_2.BUF[mb].ID.B.STD_ID = (uint16_t)id; /* MB ID */
    }
    CAN_2.BUF[mb].CS.B.CODE = CAN_RX_EMPTY; /* Set rx MB empty */
}
...
*
******************************************************************************/
void CanTxMsg2 (uint32_t id, uint8_t mb, uint8_t dlc, uint8_t data[], uint8_t ext)
{
    uint8_t	i;

    if (ext)
    {
        CAN_2.BUF[mb].CS.B.IDE = 1; /* extended ID */
        CAN_2.BUF[mb].ID.R = id;    /* Tx ID extended */
    }
    else
    {
        CAN_2.BUF[mb].CS.B.IDE = 0; /* standard ID */
        CAN_2.BUF[mb].ID.B.STD_ID = (uint16_t)id; /* Tx ID standard */
    }
    CAN_2.BUF[mb].CS.B.RTR = 0;      /* no remote Tx request frame */
    CAN_2.BUF[mb].CS.B.LENGTH = dlc; /* # bytes to Tx */
    for (i = 0; i < dlc; i++) 
    {
        CAN_2.BUF[mb].DATA.B[i] = data[i]; /* Data to Tx */
    }
    CAN_2.BUF[mb].CS.B.SRR = 1;            /* Tx frame */
    CAN_2.BUF[mb].CS.B.CODE = CAN_TX_MSG;  /* Send msg */ 
}
...
*
******************************************************************************/
can_msg_struct CanRxMsg2 (uint8_t mb)
{
	uint8_t	i;
	uint32_t dummy;
	uint32_t flagIFRL = 0;
    ...
    msg.code   = (uint8_t)CAN_2.BUF[mb].CS.B.CODE;
    if (CAN_2.BUF[0].CS.B.IDE) /* if extended ID */
    {
        msg.id  = CAN_2.BUF[mb].ID.R;
    }
    else /* if standard ID */
    {
        msg.id  = (uint16_t)CAN_2.BUF[mb].ID.B.STD_ID;
    }
    msg.length = (uint8_t)CAN_2.BUF[mb].CS.B.LENGTH;
    for (i = 0; i < msg.length; i++) 
    { 
        msg.data[i] = CAN_2.BUF[mb].DATA.B[i];
    }
    dummy = CAN_2.TIMER.R;  /* Read TIMER to unlock MB */    
    CAN_2.IFRL.R = flagIFRL; /* Clear MB flag */
    CAN_2.IFRH.R = flagIFRH; /* Clear MB flag */
    return (msg);
}
/******************************************************************************
*   Function: CanRxMbFull
...
*
******************************************************************************/
uint8_t CanRxMbFull2 (uint8_t mb)
{
	uint8_t	temp = 0;
	uint32_t flagIFRL = 0;
	uint32_t flagIFRH = 0;
...
*
******************************************************************************/
uint8_t CanTxMbEmpty2 (uint8_t mb)
{
    uint8_t	temp = 0;

    if (CAN_2.BUF[mb].CS.B.CODE == TX_CODE_INACTIVE ||
        CAN_2.BUF[mb].CS.B.CODE == RX_CODE_INACTIVE) 
    {
        temp = 1;    
    }
    return (temp);
...
*
******************************************************************************/
void SetCanRxFilter3(uint32_t id, uint8_t mb, uint8_t ext) 
{
    if (ext)
    {
        CAN_3.BUF[mb].CS.B.IDE = 1;/* MB for extended ID */
        CAN_3.BUF[mb].ID.R = id;   /* MB ID */
    }
    else
    {
        CAN_3.BUF[mb].CS.B.IDE = 0; /* MB for standard ID */
        CAN_3.BUF[mb].ID.B.STD_ID = (uint16_t)id; /* MB ID */
    }
    CAN_3.BUF[mb].CS.B.CODE = CAN_RX_EMPTY; /* Set rx MB empty */
}
...
*
******************************************************************************/
void CanTxMsg3 (uint32_t id, uint8_t mb, uint8_t dlc, uint8_t data[], uint8_t ext)
{
    uint8_t	i;

    if (ext)
    {
        CAN_3.BUF[mb].CS.B.IDE = 1; /* extended ID */
        CAN_3.BUF[mb].ID.R = id;    /* Tx ID extended */
    }
    else
    {
        CAN_3.BUF[mb].CS.B.IDE = 0; /* standard ID */
        CAN_3.BUF[mb].ID.B.STD_ID = (uint16_t)id; /* Tx ID standard */
    }
    CAN_3.BUF[mb].CS.B.RTR = 0;      /* no remote Tx request frame */
    CAN_3.BUF[mb].CS.B.LENGTH = dlc; /* # bytes to Tx */
    for (i = 0; i < dlc; i++) 
    {
        CAN_3.BUF[mb].DATA.B[i] = data[i]; /* Data to Tx */
    }
    CAN_3.BUF[mb].CS.B.SRR = 1;            /* Tx frame */
    CAN_3.BUF[mb].CS.B.CODE = CAN_TX_MSG;  /* Send msg */ 
}
...
*
******************************************************************************/
can_msg_struct CanRxMsg3 (uint8_t mb)
{
	uint8_t	i;
	uint32_t dummy;
	uint32_t flagIFRL = 0;
    ...
    msg.code   = (uint8_t)CAN_3.BUF[mb].CS.B.CODE;
    if (CAN_3.BUF[0].CS.B.IDE) /* if extended ID */
    {
        msg.id  = CAN_3.BUF[mb].ID.R;
    }
    else /* if standard ID */
    {
        msg.id  = (uint16_t)CAN_3.BUF[mb].ID.B.STD_ID;
    }
    msg.length = (uint8_t)CAN_3.BUF[mb].CS.B.LENGTH;
    for (i = 0; i < msg.length; i++) 
    { 
        msg.data[i] = CAN_3.BUF[mb].DATA.B[i];
    }
    dummy = CAN_3.TIMER.R;  /* Read TIMER to unlock MB */    
    CAN_3.IFRL.R = flagIFRL; /* Clear MB flag */
    CAN_3.IFRH.R = flagIFRH; /* Clear MB flag */
    return (msg);
}
/******************************************************************************
*   Function: CanRxMbFull
...
*
******************************************************************************/
uint8_t CanRxMbFull3 (uint8_t mb)
{
	uint8_t	temp = 0;
	uint32_t flagIFRL = 0;
	uint32_t flagIFRH = 0;
...
*
******************************************************************************/
uint8_t CanTxMbEmpty3 (uint8_t mb)
{
    uint8_t	temp = 0;

    if (CAN_3.BUF[mb].CS.B.CODE == TX_CODE_INACTIVE ||
        CAN_3.BUF[mb].CS.B.CODE == RX_CODE_INACTIVE) 
    {
        temp = 1;    
    }
    return (temp);
