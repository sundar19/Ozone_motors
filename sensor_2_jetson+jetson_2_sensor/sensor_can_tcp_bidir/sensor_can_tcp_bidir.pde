...
*/
#define	node1can1	0x5A6L //rxCAN1()
#define node2can1	0x01L //txCAN2()
#define node1can2	0x626L //change txCAN1() Sender node ID high priority message
#define node2can2	0x02L //rxCAN2()

#define SYS_FREQ	(80000000L)
#define CAN_BUS_SPEED   500000		// CAN Speed change
...
byte rgbRead[1024];
byte customWrite[]= {'a','c'}; //change here mind the size of bytes
int customSize = sizeof(customWrite);
byte dataWrite[]= {'a','b','c','d','e','g','h','i'};   //change here mind the size of bytes
int dataSize = sizeof(dataWrite);
byte wrongWrite[]= {'W','R','O','N','G'};
int wrongSize = sizeof(wrongWrite);
byte canWrite[1024]; 
int canSize = sizeof(canWrite);
int cbRead = 0;
int count = 0;
int x;
...
loop() {
 ...
 /*   ctrl = Serial.parseInt(); 
   if ( ctrl == 1)
   {
    ...
    if((canSize = tcpClient.available()) > 0)
        {
            canSize = canSize < sizeof(canWrite) ? canSize : sizeof(canWrite);
            canSize = tcpClient.readStream(canWrite, canSize);
        ...
        {               
            ...
            {
                Serial.println((char) canWrite[i]);
            }
            if (canWrite[0]=='a')
            {
          txCAN2(node2can1);
...
txCAN1(uint32_t rxnode) {
  ...
  if (message != NULL) {
    ...
    message->msgSID.SID   = rxnode;	//receiving node rxnode	change (sender node only)
    message->msgEID.IDE   = 0;			
    message->msgEID.DLC   = 8;	//data length		
    message->data[0]        = 0x40; //SDO expedited read command
    message->data[1]        = 0x4D; // index of OD entry
    message->data[2]        = 0x32; // sub index of OD entry
    message->data[3]        = 0x00;
    message->data[4]        = 0x00;
    message->data[5]        = 0x00;
    message->data[6]        = 0x00;
...
rxCAN1(void) {
 int a,b,c;
  ...
  /*
   ...
   Serial.print("\t");
   Serial.print(int(message->data[1]));
   Serial.print("\t");
   Serial.print(int(message->data[2]));
   Serial.print("\t");
   Serial.print(int(message->data[3]));
   Serial.print("\t");
   Serial.print(int(message->data[4]));
   Serial.print("\t");
   Serial.print(int(message->data[5]));
   Serial.print("\t");
   Serial.print(int(message->data[6]));
   Serial.print("\t");
   Serial.print(int(message->data[7]));
   Serial.print("\t");
  */ 
   a = int(message->data[4]);
   b = int(message->data[5]);
   c = b << 8 | a ;
   customWrite[0] = byte(message->data[4]);
   customWrite[1]= byte(message->data[5]);
   Serial.print(int(message->data[4]));
   Serial.print("\t");
   Serial.print(int(message->data[5]));
   Serial.print("\nKey_switch_voltage:");
